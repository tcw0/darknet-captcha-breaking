...
OUTPUT_TRAINING_FOLDER = os.path.join(os.path.dirname(__file__), f"data{os.sep}training_data")
OUTPUT_VALIDATION_FOLDER = os.path.join(os.path.dirname(__file__), f"data{os.sep}validation_data")
...
def create():
    ...
    X_train = permuted_image_list[:int(split_rate * len(permuted_image_list))]
    X_test = permuted_image_list[int(split_rate * len(permuted_image_list)):]
    Y_train = permuted_label_list[:int(split_rate * len(permuted_image_list))]
    Y_test = permuted_label_list[int(split_rate * len(permuted_image_list)):]
